...
// Structură pentru pachetele care așteaptă un răspuns ARP
...
struct trie_node *create_trie_node()
{
	...
	if (!node) {
		perror("Eroare la alocarea nodului trie");
		exit(1);
	}
...
/* handle_icmp_msg – sends Echo Reply */
void handle_icmp_msg(char *buf, size_t len, size_t interface)
{
	...
	if (icmp->mtype == ICMP_ECHO) {
		...
		// Recalculăm checksum-ul headerului IP
		ip_reply->checksum = 0;
...
/* handle_ttl_expriation – sends ICMP Time Exceeded */
void handle_ttl_expiration(char *buf, size_t len, size_t interface)
{
	...
	// Construim noul antet IP în buffer
	...
	// Construim antetul ICMP pentru "Time Exceeded"
	struct icmp_hdr *icmp_reply = (struct icmp_hdr *)(buf + sizeof(struct ether_hdr) + sizeof(struct ip_hdr));
...
/* handle_dest_unreachable – sends ICMP Destination Unreachable */
void handle_dest_unreachable(char *buf, size_t len, size_t interface)
{
	...
	// Actualizăm antetul Ethernet: interschimbăm adresele MAC
	uint8_t temp_mac[6];
